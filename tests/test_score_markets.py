...
for name, pa, pb in CASES:
    for bo in (3, 5):
        ...
        t = sc.fair_line(totals, "total", bo)
        h = sc.fair_line(margins, "margin", bo)
        check(f"{name} bo{bo}: total {t} and handicap {-h} are half-points",
...
for name, pa, pb in CASES:
    ...
    for kind, dist in (("total", totals), ("margin", margins)):
        ...
        back = sc.uncalibrate(sc.calibrate(v, dist, kind, 3), dist, kind, 3)
...
# the distribution beside it said something else, because the two were computed
# through different calibrations.
for name, pa, pb in CASES:
    for bo in (3, 5):
        d = mk.match_distribution(pa, pb, bo)
        totals = mk.total_games_distribution(d["games"])
        ...
        line = sc.fair_line(totals, "total", bo)
        p_over = mk.prob_over_games(
            d["games"], sc.uncalibrate(line, totals, "total", bo))
        check(f"{name} bo{bo}: P(over fair total) near 0.5 (={p_over:.3f})",
              abs(p_over - 0.5) < 0.10, f"{p_over:.4f}")

        hcap = -sc.fair_line(margins, "margin", bo)
        raw = sc.uncalibrate(-hcap, margins, "margin", bo)
        p_cov = mk.prob_cover_handicap(d["games"], -raw)
        check(f"{name} bo{bo}: P(cover fair handicap) near 0.5 (={p_cov:.3f})",
              abs(p_cov - 0.5) < 0.10, f"{p_cov:.4f}")
...
check("a favourite gives up more games than an even player",
      -sc.fair_line(m_fav, "margin", 3) < -sc.fair_line(m_even, "margin", 3))
# An even match is long; a mismatch is short.
t_even = sc.fair_line(mk.total_games_distribution(d_even["games"]), "total", 3)
t_fav = sc.fair_line(mk.total_games_distribution(d_fav["games"]), "total", 3)
...
t3 = sc.fair_line(mk.total_games_distribution(d_even["games"]), "total", 3)
d5 = mk.match_distribution(0.64, 0.64, 5)
t5 = sc.fair_line(mk.total_games_distribution(d5["games"]), "total", 5)
...
for name, pa, pb in CASES:
    t = sc.fair_line(mk.total_games_distribution(
        mk.match_distribution(pa, pb, 3)["games"]), "total", 3)
...
for kind in ("total", "margin"):
    for bo in (3, 5):
        check(f"{kind} bo{bo} has a centre and a spread",
              bo in sc.CENTRE[kind] and bo in sc.SPREAD[kind])
...
check("spreads are plausible widths (0.5-2.0)",
      all(0.5 < s < 2.0 for k in sc.SPREAD for s in sc.SPREAD[k].values()))
